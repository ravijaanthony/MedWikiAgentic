...
import { startConsultation } from "../api/client";
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to start consultation. Ensure backend is running on :8000"
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-2xl">
      ...
      <p className="text-sm text-slate-600 mt-1">
        Ambient pipeline: VALSEA → Refine → Dispatch → Specialists → Integrity
      </p>
      ...
      <div className="mt-6 bg-white rounded-xl border border-slate-200 p-6 space-y-4">
        ...
        {mode === "fixture" && (
          <p className="text-xs text-slate-500 bg-slate-50 rounded p-3">{FIXTURE_HINT}</p>
        )}
